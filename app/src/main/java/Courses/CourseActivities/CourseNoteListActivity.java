...
public class CourseNoteListActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private static final int COURSE_NOTE_DETAIL_ACTIVITY_CODE = 22222;
    ...
    private void bindCourseNoteList() {
        String[] from = {DataHelper.COURSE_NOTE_TEXT};
        int[] to = {R.id.courseNoteText};
        cursorAdapter = new SimpleCursorAdapter(this, R.layout.list_item_course_note, null, from, to, 0);
        DataProvider database = new DataProvider();

        ListView list = (ListView) findViewById(R.id.courseNoteList);
        list.setAdapter(cursorAdapter);
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent(CourseNoteListActivity.this, CourseNoteDetailActivity.class);
                Uri uri = Uri.parse(DataProvider.COURSE_NOTES_URI + "/" + id);
                intent.putExtra(DataProvider.COURSE_NOTE_CONTENT_TYPE, uri);
                startActivityForResult(intent, COURSE_NOTE_DETAIL_ACTIVITY_CODE);
            }
        });
    }
    ...
    public Loader<Cursor> onCreateLoader(int id, Bundle args) {
        return new CursorLoader(this, DataProvider.COURSE_NOTES_URI, DataHelper.COURSE_NOTES_COLUMNS, DataHelper.COURSE_NOTE_COURSE_ID + " = " + this.courseId, null, null);
    }
